...
clog.switchState = function (state,arg) {
  ...
  $(window).off('scroll.clog');
    ...
    var createPostCallback = function (post) {
      ...
      clog.utils.renderTemplate('create_post', templateData, 'clog_content');
        ...
        $(document).ready(function () {
          ...
          $('#clog_title_field').bind('keypress', () => clog.titleChanged = true);

          $('#clog_visibility_maintainer,#clog_visibility_site').click(() => $('#clog-group-fieldset').hide());
          ...
          $('#clog-visibility-group').click(() => $('#clog-group-fieldset').show());

          $('#clog_save_post_button').click(() => clog.utils.savePostAsDraft(clog.editor));

          $('#clog_make_post_public_button').click(() => clog.utils.publicisePost(clog.editor));

          $('#clog_publish_post_button').click(() => clog.utils.publishPost(clog.editor));
          ...
          clog.autosave_id = setInterval(() => clog.utils.autosavePost(clog.editor), 10000);
    ...
    clog.utils.renderTemplate('permissions', {}, 'clog_content');
    ...
    $('#clog_recycle_bin_link > span').addClass('current');
      ...
      success: function (data) {
        ...
        posts.forEach(p => clog.utils.renderPost(p, 'post_' + p.id));
...
clog.init = function () {
  ...
  if ($(document).width() < 800) {
    ...
    if (clog.groups.length > 0) {
      $("#clog_groups_link").show().click(() => clog.switchState('groups'));
    }

    $('#clog_home_link>span>a').click(() => clog.switchState('home'));

    $('#clog_view_authors_link>span>a').click(() => clog.switchState('viewMembers'));

    $('#clog_my_clog_link>span>a').click(() => clog.switchState('userPosts', {userDisplayName: clog.userDisplayName}));
    ...
    $('#clog_create_post_link>span>a').click(() => clog.switchState('createPost'));

    $('#clog_permissions_link>span>a').click(() => clog.switchState('permissions'));

    $('#clog_recycle_bin_link>span>a').click(() => clog.switchState('viewRecycled'));
  }
